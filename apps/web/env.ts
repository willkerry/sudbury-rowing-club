import { createEnv } from "@t3-oss/env-nextjs";
import { z } from "zod";

export const env = createEnv({
  client: {
    NEXT_PUBLIC_ALGOLIA_APP_ID: z.string(),
    NEXT_PUBLIC_ALGOLIA_INDEX_NAME: z.string(),
    NEXT_PUBLIC_ALGOLIA_SEARCH_KEY: z.string(),
    NEXT_PUBLIC_MAPTILER_KEY: z.string(),
    NEXT_PUBLIC_POSTHOG_HOST: z.string(),
    NEXT_PUBLIC_POSTHOG_KEY: z.string(),
    NEXT_PUBLIC_SANITY_DATASET: z.string(),
    NEXT_PUBLIC_SANITY_PROJECT_ID: z.string(),
    NEXT_PUBLIC_TURNSTILE_SITE_KEY: z.string(),
    NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA: z.string().optional(),
  },
  runtimeEnv: {
    ALGOLIA_API_KEY: process.env.ALGOLIA_API_KEY,
    APP_URL: process.env.APP_URL,
    BUG_RECIPIENT_EMAIL: process.env.BUG_RECIPIENT_EMAIL,
    FIRECRAWL_API_KEY: process.env.FIRECRAWL_API_KEY,
    KV_REST_API_TOKEN: process.env.KV_REST_API_TOKEN,
    KV_REST_API_URL: process.env.KV_REST_API_URL,
    NEXT_PUBLIC_ALGOLIA_APP_ID: process.env.NEXT_PUBLIC_ALGOLIA_APP_ID,
    NEXT_PUBLIC_ALGOLIA_INDEX_NAME: process.env.NEXT_PUBLIC_ALGOLIA_INDEX_NAME,
    NEXT_PUBLIC_ALGOLIA_SEARCH_KEY: process.env.NEXT_PUBLIC_ALGOLIA_SEARCH_KEY,
    NEXT_PUBLIC_MAPTILER_KEY: process.env.NEXT_PUBLIC_MAPTILER_KEY,
    NEXT_PUBLIC_POSTHOG_HOST: process.env.NEXT_PUBLIC_POSTHOG_HOST,
    NEXT_PUBLIC_POSTHOG_KEY: process.env.NEXT_PUBLIC_POSTHOG_KEY,
    NEXT_PUBLIC_SANITY_DATASET: process.env.NEXT_PUBLIC_SANITY_DATASET,
    NEXT_PUBLIC_SANITY_PROJECT_ID: process.env.NEXT_PUBLIC_SANITY_PROJECT_ID,
    NEXT_PUBLIC_TURNSTILE_SITE_KEY: process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY,
    NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA:
      process.env.NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA,
    NODE_ENV: process.env.NODE_ENV,
    REDIRECT_SECRET: process.env.REDIRECT_SECRET,
    REGATTA_STATE_BRANCH: process.env.REGATTA_STATE_BRANCH,
    REGATTA_STATE_REPO: process.env.REGATTA_STATE_REPO,
    REGATTA_WEBHOOK_SECRET: process.env.REGATTA_WEBHOOK_SECRET,
    RESEND_API_KEY: process.env.RESEND_API_KEY,
    RESEND_WEBHOOK_SECRET: process.env.RESEND_WEBHOOK_SECRET,
    TURNSTILE_SECRET_KEY: process.env.TURNSTILE_SECRET_KEY,
    VERCEL_ENV: process.env.VERCEL_ENV,
  },
  server: {
    ALGOLIA_API_KEY: z.string().min(1),
    BUG_RECIPIENT_EMAIL: z.string(),
    FIRECRAWL_API_KEY: z.templateLiteral(["fc-", z.string()]),
    KV_REST_API_TOKEN: z.string().min(1),
    KV_REST_API_URL: z.url(),
    REDIRECT_SECRET: z.string().min(32),
    REGATTA_STATE_BRANCH: z.string().optional(),
    REGATTA_STATE_REPO: z.string().optional(),
    REGATTA_WEBHOOK_SECRET: z.string().min(16).optional(),
    RESEND_API_KEY: z.string().min(1),
    RESEND_WEBHOOK_SECRET: z.string().min(1),
    TURNSTILE_SECRET_KEY: z.string().min(1),
  },
  shared: {
    APP_URL: z.string().optional(),
    NODE_ENV: z.enum(["development", "production"]),
    VERCEL_ENV: z.enum(["development", "preview", "production"]).optional(),
  },
});
